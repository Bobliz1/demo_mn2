...
- **MN-BO 单次开销**：
  ...
  - 外层搜索 5 次：5 × 8 = 40 步
  - MN-BO 合计：104 步
- **基线 BO 开销**：获得等同的 104 步迭代机会。
- **12 函数全跑（R=5）总计：** 12 × (104 + 104 + 5(全局初始化)) × 5 = 12,780 次评估

**完整实验模式**（需显式指定旧默认参数 `-R 5 -M 20 -N 12 -I 36`）：
- **MN-BO 单次开销**：
  - 外层初始 36 点：36 × 12 = 432 步
  - 外层搜索 20 次：20 × 12 = 240 步
  - MN-BO 合计：672 步
- **基线 BO 开销**：获得等同的 672 步迭代机会。
- **12 函数全跑（R=5）总计：** 12 × (672 + 672 + 5) × 5 = 80,940 次评估
